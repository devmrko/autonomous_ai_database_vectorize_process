...
def show_status(conn):
    ...
    try:
        ...
        if CONFIG["credential_name"] in creds:
            print(f"  ✅ {CONFIG['credential_name']}")
        else:
            print(f"  ❌ {CONFIG['credential_name']} (not found)")
    except Exception:
        print("  ⚠️  Unable to check credentials")
    ...
    try:
        ...
        if CONFIG["embed_profile"] in profiles:
            print(f"  ✅ {CONFIG['embed_profile']}")
        else:
            print(f"  ❌ {CONFIG['embed_profile']} (not found)")
    except Exception:
        print("  ⚠️  Unable to check AI profiles")
...
def main():
    ...
    try:
        ...
        if args.drop_all:
            confirm = input("⚠️  This will DROP ALL tables and data! Type 'YES' to confirm: ")
            if confirm == "YES":
                drop_all(conn)
            else:
                print("Aborted.")
        ...
        if args.reset:
            confirm = input("⚠️  This will RESET everything! Type 'YES' to confirm: ")
            if confirm == "YES":
                drop_all(conn)
                create_tables(conn)
